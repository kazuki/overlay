...
DRIVERS_URI="http://www2.ati.com/drivers/linux/amd-catalyst-15.9-linux-installer-15.201.1151-x86.x86_64.zip"
...
src_unpack() {
	local DRIVERS_DISTFILE XVBA_SDK_DISTFILE RUN_FILE
	...
	if [[ ${DRIVERS_DISTFILE} =~ .*\.tar\.gz ]]; then
		...
		if [[ ${DRIVERS_DISTFILE} =~ .*\.zip ]]; then
			unpack ${DRIVERS_DISTFILE}
            RUN_FILE=${DRIVERS_DISTFILE/%.zip/.run}
            RUN_FILE=${RUN_FILE/amd-c/AMD-C}
            RUN_FILE=${RUN_FILE/linux/Linux}
			[[ -z "$RUN" ]] && RUN="${S}/${RUN_FILE}"
		else
...
src_prepare() {
    ...
    # Comple fix for linux 4.[012]
    epatch "${FILESDIR}/linux-4.0.patch"
    epatch "${FILESDIR}/linux-4.1.patch"
    epatch "${FILESDIR}/linux-4.2.patch"
